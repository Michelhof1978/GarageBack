...
<?php
$imageCriterePreview = ''; // Initialisation de la variable pour l'image de critère
$imageVoiturePreview = ''; // Initialisation de la variable pour l'image de voiture
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Gérez le téléchargement de l'image de critère
    if (isset($_FILES['imageCritere']) && $_FILES['imageCritere']['error'] === UPLOAD_ERR_OK) {
        $tmpFile = $_FILES['imageCritere']['tmp_name'];

        // Gérez le téléchargement de l'image de critère
        $fileType = mime_content_type($tmpFile);
        if (strpos($fileType, 'image') === 0) {
            $destinationDir = 'votre_repertoire_de_destination/';
            $fileName = uniqid('image_critere_') . '.' . pathinfo($_FILES['imageCritere']['name'], PATHINFO_EXTENSION);
            $destinationPath = $destinationDir . $fileName;

            if (move_uploaded_file($tmpFile, $destinationPath)) {
                $imageCriterePreview = $destinationPath;
            }
        }
    }

    // Gérez le téléchargement de l'image de voiture
    if (isset($_FILES['imageVoiture']) && $_FILES['imageVoiture']['error'] === UPLOAD_ERR_OK) {
        $tmpFile = $_FILES['imageVoiture']['tmp_name'];

        // Gérez le téléchargement de l'image de voiture
        $fileType = mime_content_type($tmpFile);
        if (strpos($fileType, 'image') === 0) {
            $destinationDir = 'votre_repertoire_de_destination/';
            $fileName = uniqid('image_voiture_') . '.' . pathinfo($_FILES['imageVoiture']['name'], PATHINFO_EXTENSION);
            $destinationPath = $destinationDir . $fileName;

            if (move_uploaded_file($tmpFile, $destinationPath)) {
                $imageVoiturePreview = $destinationPath;
            }
        }
    }

    // Continuez avec le reste du traitement, y compris l'insertion en base de données
}
?>
...
<form method="POST" action="<?= URL ?>back/espacepro/creationvoituresoccasions" enctype="multipart/form-data">
    ...
    <!-- Champ d'upload d'image de critère -->
...
<div class="form-group">
    <img src="<?= isset($imagePreview) ? $imagePreview : '' ?>" alt="Image Preview" style="width:100px;" />
...
<div class="form-group">
  ...
  <div class="form-group">
  <img src="<?= URL ?>public/images/<?= $vehicule['imageCritere'] ?>" style="width:100px;" />
