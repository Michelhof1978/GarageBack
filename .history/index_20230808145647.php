...
try{
    if(empty($_GET['page'])){
        ...
        if(empty($url[0]) || empty($url[1])) throw new Exception ("La page n'existe pas");//On va vérifier que l'URL contient bien 2 paramètres en ajoutant une auytre exeption.Si .
        switch($url[0]){
            case "front" : 
                switch($url[1]){
