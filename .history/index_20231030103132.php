...
$prestationManager = new PrestationManager(); // Instanciez le gestionnaire de prestation avant le contrôleur
...
try {
    if (empty($_GET['page'])) {
        ...
        if (empty($url[0]) || empty($url[1])) throw new Exception("La page n'existe pas"); // On va vérifier que l'URL contient bien 2 paramètres en ajoutant une autre exception. Si à l'indice 0 ou 1 de l'URL après le / n'existe pas après la page front ou back, dans ce cas-là je vais lever une erreur.
        switch ($url[0]) {
            case "front": // On vérifie la valeur de l'URL 0, si elle est égale à front, on va vérifier la valeur de l'URL 1.
                switch ($url[1]) { // On vérifie la valeur de l'URL 1, si elle est égale à accueil, on affiche la page accueil.
                    case "voiturefiche": // On affiche la page voitureFiche et on ajoute à l'indice 2 la voiture sélectionnée avec un Id.
                    ...
                    case "contact": $contact_controller->getContact();
                        break;
                    case "avis": $avis_controller->getAvisVerifiés($avisManager); // Utilisez le bon nom de méthode
                        break;
            ...
            case "back":
                ...
                break;
            default: throw new Exception("La page n'existe pas"); // On lève encore une exception que s'il n'y a pas front ou back mais autre chose de marqué dans l'URL, on affichera un message d'erreur.
        }
    }
} catch (Exception $e) {
    $msg = $e->getMessage();
