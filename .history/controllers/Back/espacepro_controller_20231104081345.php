...
public function getAvisNonValides()
{
    if (Securite::verifAccessSession()) {
        ...
        require_once(__ROOT__ . '/views/commons/espacepro_avis_non_valides_view.php'); // Créez une vue pour afficher les avis non validés
    } else {
        throw new Exception("Vous n'avez pas accès à cette page");
    }
}
...
public function modificationprestation() {
    if (Securite::verifAccessSession()) {
        ...
        $this->espaceproManager->updatePrestation(
            $idPrestation, $imagePrestation, $nom, $description, $prix, $updated_at
        );

        $_SESSION['alert'] = [
            "message" => "La prestation a bien été modifié",
            "type" => "alert-success"
        ];
...
public function creationTemplatePrestation() {
    if (Securite::verifAccessSession()) {
        require_once "views/commons/espacepro_ajout_prestation_view.php";
    } else {
        throw new Exception("Vous n'avez pas accès à cette page");
    }
}
...
public function creationprestation() {
    if (Securite::verifAccessSession()) {
        try {
            $imagePrestation = "";
            if (isset($_FILES['imagePrestation']['size']) && $_FILES['imagePrestation']['size'] > 0) {
                $repertoire = "public/images/";
                $imagePrestation = ajoutImage($_FILES['imagePrestation'], $repertoire);
            }
            ...
            $idPrestation = $this->espaceproManager->createPrestation(
                $imagePrestation, $nom, $description, $prix, $created_at
            );

            $_SESSION['alert'] = [
                "message" => "La prestation a bien été créée sous l'identifiant : " . $idPrestation,
                "type" => "alert-success"
            ];

            header('Location: ' . URL . 'back/espacepro/creationtemplateprestation');
            exit();
        } catch (Exception $e) {
            $_SESSION['alert'] = [
