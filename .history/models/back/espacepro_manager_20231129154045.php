...
class EspaceproManager extends Model
{
    ...
    public function deleteDBvehicule($idVehicule)
    {
        try {
            ...
            $stmt->bindValue(":idVehicule", $idVehicule, PDO::PARAM_INT); //Ne pas oublier de le convertir en INT car les formulaire retourne automatiquement des chaine de caractère.
    ...
    public function updateVehicule(updateVehicule $imageVoiture, $famille, $marque, $modele, $annee, $kilometrage, $boitevitesse, $energie, $datecirculation, $puissance, $places, $couleur, $description, $prix, $imageCritere, $updated_at)
    {
    ...
    public function getimagePrestation($idPrestation)
    {
        $req = "SELECT imagePrestation from prestation where idPrestation = :idPrestation";
        $stmt = $this->getBdd()->prepare($req);
        $stmt->bindValue(":idPrestation", $idPrestation, PDO::PARAM_INT);
        $stmt->execute();
        $image = $stmt->fetch(PDO::FETCH_ASSOC);
        $stmt->closeCursor();
        return $image['imagePrestation'];
    }

    ////////////FIN SUPPRESSION VEHICULE

    // MODIFICATION PRESTATION
    public function updatePrestation($idPrestation, $imagePrestation, $nom, $description, $prix, $updated_at)
    {

        $updated_at = date("Y-m-d H:i:s"); //Mettre la date actuelle

        $req = "UPDATE prestation SET 
                imagePrestation = :imagePrestation, 
                nom = :nom,
                description = :description,
                prix = :prix,  
                updated_at = :updated_at
