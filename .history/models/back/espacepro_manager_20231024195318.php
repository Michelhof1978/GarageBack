...
public function compterAvis($idAvis){
    $req = "SELECT COUNT(*) AS nb FROM vehicule WHERE idAvis = :idVehicule"; 
    $stmt = $this->getBdd()->prepare($req);
    $stmt->bindValue(":idVehicule", $idAvis, PDO::PARAM_INT);//Ne pas oublier de le convertir en INT car les formulaire sont automatiquement en string
    if ($stmt->execute()) {
        $resultat = $stmt->fetch(PDO::FETCH_ASSOC);
        $stmt->closeCursor();
        return ($resultat) ? $resultat['nb'] : 0;
