...
class EspaceproManager extends Model {
    ...
    public function updateVehicule($idVehicule,$imageVoiture, $famille, $marque, $model, $annee, $kilometrage, $boitevitesse, $energie, $datecirculation, $puissance, $places, $couleur, $description, $prix, $datecirculation, $imageCritere){
        $req = "UPDATE FROM `vehicule`set idVehicule";
        $stmt = $this->getBdd()->prepare($req);
        $stmt->bindValue(":idVehicule", $idVehicule, PDO::PARAM_INT);
        $stmt->execute();
        $stmt->closeCursor();
